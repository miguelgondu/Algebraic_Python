	...
	def getSubgroupGeneratedBySubset(self, subset):
		...
		while Queue_list != []:
			...
			if q not in Subgroup:
				Subgroup.append(q)
				for element in subset:
					Queue_list.append(self.operate(q,element))

		return set(Subgroup)

	def getAllSubgroups(self):
		'''
		Returns a list with all subgroups of a group.

		To-Do:
		- Rethink this algorithm, it was implemented by pure force.
		'''
		_powerset = ST.getPowerSet(self.Group_set)
		list_of_subgroups = []
		for subset in _powerset:
			subgroup_generated_by_subset = self.getSubgroupGeneratedBySubset(subset)
			if subgroup_generated_by_subset not in list_of_subgroups:
				list_of_subgroups.append(subgroup_generated_by_subset)
